# -*- coding: utf-8 -*-
"""
测试目的：验证 torch.distributed.Work.unbox 接口功能正确性
API 名称：torch.distributed.Work.unbox
API 签名：Work.unbox()

覆盖维度表：
| 覆盖维度         | 说明                                | 覆盖情况                |
|------------------|-------------------------------------|-------------------------|
| 方法存在         | unbox 方法可调用                    | 已覆盖                  |
...
def _worker(rank, ws, dn):
    """Worker: verify unbox is callable on Work object."""
...
class TestWorkUnbox(TestCase):
    ...
    def test_unbox_2npu(self):
        """Verify unbox does not raise across 2 NPU processes."""
