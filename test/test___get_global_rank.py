# -*- coding: utf-8 -*-
"""
测试目的：验证 torch.distributed.get_global_rank 接口功能正确性
API 名称：torch.distributed.get_global_rank
...
def _worker(rank, ws, dn):
    """Worker: create subgroup and verify get_global_rank returns int."""
    g = dist.new_group(ranks=list(range(ws)))
    r = dist.get_global_rank(g, rank)
    assert isinstance(r, int)


class TestGetGlobalRank(TestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_2npu(self):
        """Verify get_global_rank returns int across 2 NPU processes."""
