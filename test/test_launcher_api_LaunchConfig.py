...
| 异常传参场景     | 无稳定异常路径                                   | 未覆盖：纯 dataclass 无校验                     |
...
class TestLauncherApiLaunchConfig(TestCase):
    """Test cases for torch.distributed.launcher.api.LaunchConfig."""
    ...
    def test_required_args_only(self):
        """Construct with only required args, verify defaults."""
    ...
    def test_default_rdzv_endpoint(self):
        """rdzv_endpoint defaults to empty string."""
        config = LaunchConfig(min_nodes=1, max_nodes=1, nproc_per_node=1)
        self.assertEqual(config.rdzv_endpoint, "")

    def test_default_rdzv_configs(self):
        """rdzv_configs defaults to empty dict."""
        config = LaunchConfig(min_nodes=1, max_nodes=1, nproc_per_node=1)
        self.assertIsInstance(config.rdzv_configs, dict)
        self.assertIn("timeout", config.rdzv_configs)

    def test_default_rdzv_timeout(self):
        """rdzv_timeout=-1 triggers default timeout in post_init."""
        config = LaunchConfig(min_nodes=1, max_nodes=1, nproc_per_node=1)
        self.assertEqual(config.rdzv_configs.get("timeout"), 900)

    def test_custom_rdzv_timeout(self):
        """Custom rdzv_timeout sets timeout in rdzv_configs."""
        config = LaunchConfig(
    ...
    def test_custom_role(self):
        """Custom role is preserved."""
        config = LaunchConfig(
            min_nodes=1, max_nodes=1, nproc_per_node=1,
            role="trainer"
        )
        self.assertEqual(config.role, "trainer")

    def test_custom_rdzv_backend(self):
        """Custom rdzv_backend is preserved."""
        config = LaunchConfig(
            min_nodes=1, max_nodes=1, nproc_per_node=1,
            rdzv_backend="c10d"
        )
        self.assertEqual(config.rdzv_backend, "c10d")

    def test_custom_metrics_cfg(self):
        """Custom metrics_cfg is preserved."""
        config = LaunchConfig(
            min_nodes=1, max_nodes=1, nproc_per_node=1,
            metrics_cfg={"metric1": "value1"}
        )
        self.assertEqual(config.metrics_cfg, {"metric1": "value1"})

    def test_logs_specs_not_none(self):
        """logs_specs is set by __post_init__ even when not passed."""
        config = LaunchConfig(min_nodes=1, max_nodes=1, nproc_per_node=1)
    ...
    def test_large_nproc_per_node(self):
        """Large nproc_per_node value."""
        config = LaunchConfig(min_nodes=1, max_nodes=1, nproc_per_node=8)
    ...
    def test_multi_node_config(self):
        ...
        self.assertEqual(config.nproc_per_node, 4)
