# -*- coding: utf-8 -*-
"""
测试目的：验证 torch.amp.autocast 接口功能正确性
API 名称：torch.amp.autocast
API 签名：torch.amp.autocast(device_type, dtype=None, enabled=True, cache_enabled=True)

覆盖维度表：
| 覆盖维度         | 说明                                | 覆盖情况                |
|------------------|-------------------------------------|-------------------------|
| 基础调用         | 创建 autocast 上下文不报错          | 已覆盖                  |
| device_type      | 使用 self.device_name 构建          | 已覆盖                  |
| enabled=True     | 上下文内 autocast 启用              | 已覆盖                  |
| enabled=False    | 上下文内 autocast 禁用              | 已覆盖                  |
| dtype 参数       | 指定 float16/bfloat16              | 已覆盖                  |
| 上下文管理器     | with 语句进出状态切换               | 已覆盖                  |

未覆盖项及原因：
- cache_enabled=False：行为验证需精度对比
