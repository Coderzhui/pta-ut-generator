# -*- coding: utf-8 -*-
"""
测试目的：验证 torch.distributed.Work.exception 接口功能正确性
API 名称：torch.distributed.Work.exception
API 签名：Work.exception()

覆盖维度表：
| 覆盖维度         | 说明                                | 覆盖情况                |
|------------------|-------------------------------------|-------------------------|
| 方法存在         | exception 可调用                    | 已覆盖                  |
...
def _worker(rank, ws, dn):
    """Worker: verify exception is callable on Work object."""
    t = torch.ones(10, device=dn) * rank
    work = dist.all_reduce(t, async_op=True)
    work.wait()
    # No error occurred, exception should be None or similar
    exc = work.exception()
    assert exc is None or isinstance(exc, Exception)


class TestWorkException(TestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_exception_2npu(self):
        """Verify exception is callable on Work across 2 NPU processes."""
