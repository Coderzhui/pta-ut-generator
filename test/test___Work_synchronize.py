# -*- coding: utf-8 -*-
"""
测试目的：验证 torch.distributed.Work.synchronize 接口功能正确性
API 名称：torch.distributed.Work.synchronize
API 签名：Work.synchronize()
...
def _worker(rank, ws, dn):
    """Worker: verify synchronize completes without error."""
    t = torch.ones(10, device=dn) * rank
    work = dist.all_reduce(t, async_op=True)
    work.synchronize()


class TestWorkSynchronize(TestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_synchronize_2npu(self):
        """Verify synchronize completes without error across 2 NPU processes."""
