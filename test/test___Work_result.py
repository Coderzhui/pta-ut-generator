# -*- coding: utf-8 -*-
"""
测试目的：验证 torch.distributed.Work.result 接口功能正确性
API 名称：torch.distributed.Work.result
API 签名：Work.result()

覆盖维度表：
| 覆盖维度         | 说明                                | 覆盖情况                |
|------------------|-------------------------------------|-------------------------|
| 基础调用         | wait 后调用不报错                   | 已覆盖                  |
...
def _worker(rank, ws, dn):
    """Worker: verify result completes without error after wait."""
...
class TestWorkResult(TestCase):
    ...
    def test_result_2npu(self):
        """Verify result does not raise after wait across 2 NPU processes."""
